      ...
      {state === 'home' ? (
        ...
        <button
          className={cn(
            ...
            'rounded-lg border-[2px] border-white/30 bg-gradient-to-t from-buttonGradientFrom to-buttonGradientTo text-[16px] text-white shadow-[0_2px_6px_rgba(0.2,0.2,0.2,0.4)]',
            'cursor-pointer select-none'
          )}>
          <div onClick={onClick} className="flex items-center gap-[8px] px-[46px] py-[10px]">
            {state === 'loading' && (
              <div className={'flex h-[24px] w-[24px] items-center justify-center'}>
                <ClipLoader color={colors.loaderWhite} size={24} />
              </div>
            )}
            {state === 'splash' && <IconCamera3 width={24} height={24} className={'mb-0.5'} />}
