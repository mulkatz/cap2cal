...
export const CardController = ({ data }: { data: CaptureEvent }) => {
  const dialogs = useDialogContext();

  const img = useLiveQuery(async () => await db.images.where('id').equals(data.id).first()) || null;

  const item =
    useLiveQuery(async () => await db.eventItems.where('id').equals(data.id).first(), [db.eventItems, data]) || data;
  if (!item) return null;
